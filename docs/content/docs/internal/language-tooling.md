...
`gtsc` passes `typeCheckingOnly: true` to the language plugin. This omits
attribute-name completion receivers and expressions. It retains the hint
attributes' uniqueness probes and collected names, since these can affect a
provider's validation. Editors use the default output with completion mappings.

The generated file shares helpers for member lookup, required attributes, and
Meta merging. Conditional lookups remain non-distributive. The `~meta` fields on
`uniqueKey()` and `as()` receivers deliberately stay inline: passing Meta as a
generic argument can evaluate it too early and introduce circular inference.
The zero-key guard must likewise precede evaluation of the merged uniqueness
probe interface.

Required-attribute diagnostics map the start and end of their generated
argument to the original block's endpoints. This preserves full-block
diagnostics without padding each diagnostic string to the block's length.

To measure a project with the local toolchain:

```sh
pnpm --filter @gi-tcg/gtsc... build
node --max-old-space-size=4096 packages/tsc/scripts/benchmark.mjs \
  --noEmit --incremental false --extendedDiagnostics --pretty false \
  -p ../genius-invokation/packages/data/tsconfig.json
```

The script adds source/generated byte counts, mapping counts, and peak RSS to
TypeScript's diagnostics. Compare the same compiler, dependencies, project
options, and heap limit; peak RSS and TypeScript's reported heap usage measure
different things. If a production checkout has stale declaration output, use
a separate benchmark tsconfig with `paths` pointing at its sources, and compare
the complete diagnostics before interpreting performance results.
