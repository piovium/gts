...
`defineViewModel` 是创建 ViewModel 的核心函数。它接收一个 Model 构造函数和一个属性定义函数，返回一个配置好的 ViewModel 实例。

## `defineViewModel` 签名

```ts
function defineViewModel<ModelT, BlockDef, InitMeta>(
  Ctor: new () => ModelT,
  modelDefFn: (helper: AttributeDefHelper<ModelT>) => BlockDef,
  initMeta?: InitMeta,
): ViewModel<ModelT, BlockDef & { [Meta]: InitMeta }>;
```

- **`Ctor`** — Model 类的构造函数（`new () => ModelT`）
- **`modelDefFn`** — 属性定义函数，接收一个 helper，返回属性定义对象
- **`initMeta`** — 初始 Meta 类型（用于类型层面的状态追踪）
...
AR.With<VM, TMeta>;         // 属性打开一个嵌套 VM 块
AR.DoneRewriteMeta<NewMeta>;      // 属性重写 meta
AR.WithRewriteMeta<NewMeta, VM>;  // 打开嵌套 VM + 重写 meta
```

这些在 `TypeScript Interop` 中用于为 IDE 提供精确的类型信息，不影响运行时逻辑。
